...
3.  `02_sofa2_calculation.py`. This script calculates the SOFA score for each hospitalization in the cohort. _You are required to add timezone manually in the file, as this is expected for the SOFA-2 function_. The output dataframe contains most information needed for each hospitalization. It outputs a parquet file that is read in by `03_feature_set_processing.R`.
